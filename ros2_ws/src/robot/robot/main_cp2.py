"""
main_cp2.py - START-AT-CHECKPOINT-2 variant of main.py
======================================================
Press BTN_1 to start. BTN_2 cancels the active motion.

This is a copy of main.py that SKIPS the entire scripted cp1->bridge->cp2
sequence (MISSION_STEPS) and drops the robot straight into the LiDAR/LAPF
obstacle-avoidance leg toward checkpoint 3. Everything downstream of cp2 (LAPF
leg, watchdog/recovery, wall-square -> finish, cp3->cp4, manipulator
placeholder) is identical to main.py.

Place the robot physically at the cp2 spot facing up the obstacle course (+Y)
before pressing BTN_1: reset_mission_pose() sets odometry to the cp2 start pose
(0, 0, heading INITIAL_THETA_DEG=+Y), so the first LAPF goal aims straight up
the field. main.py is left untouched; point robot_node.py at
`from robot.main_cp2 import run` to use this build.

Route from cp2 onward (steps 1-8 of main.py's scripted bridge run are skipped):
...
# ===========================================================================
# START AT CHECKPOINT 2 -- this is the whole point of the main_cp2.py build.
# BTN_1 resets odometry to the cp2 start pose (0,0, heading +Y) and jumps
# straight into the cp2->cp3 obstacle-avoidance leg, skipping the scripted
# cp1->bridge->cp2 drive (MISSION_STEPS) entirely. The robot must be physically
# placed at the cp2 spot facing up the obstacle course (+Y) before pressing
# BTN_1.
# This is hard-wired True here on purpose; for the full cp1->cp5 scripted run
# use main.py (which keeps this False). Do not flip it back in this file.
START_AT_CP2 = True  # main_cp2.py: always start at checkpoint 2 (skip scripted steps)
# ===========================================================================
...
def run(robot: Robot) -> None:
    ...
    while True:
        ...
        if state == "INIT":
            ...
            step_index = 0
            av = {}
            print("[FSM] IDLE - press BTN_1 to start FROM checkpoint 2 (LiDAR/LAPF leg), BTN_2 to cancel")
            print(
            ...
            if robot.was_button_pressed(Button.BTN_1):
                if START_AT_CP2:
                    # main_cp2.py entry path (START_AT_CP2): skip the scripted
                    ...
                    # +Y before pressing BTN_1. (For the full scripted cp1->cp5
                    # run, use main.py instead.)
                    reset_mission_pose(robot)
                    show_running_leds(robot)
                    step_index = len(MISSION_STEPS)
                    av = {}
                    print("[FSM] START AT CHECKPOINT 2 - skipping scripted bridge steps; "
