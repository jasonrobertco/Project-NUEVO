"""
main.py - checkpoint 1->5 mission FSM + autonomous laser targeting
==================================================================
...
obstacle avoidance for the later sections:
  ...
  CP5 (vision): creep forward until the vision node sees a STOP SIGN within
    STOP_SIGN_STOP_DISTANCE_MM (70 mm), stop, then run the AUTONOMOUS laser
    targeting sequence: detect the ArUco markers (via the vision node), sort by
    nearest Z, coarse-aim from the calibration table, then close the loop with
    a measured image-Jacobian visual servo that drives the laser dot onto each
    marker center and dwells (the "shot"). All camera data for cp5 comes from
    the vision node's /vision/detections (robot.enable_vision()) -- nothing in
    this file opens /dev/video10 directly (the vision node owns the camera).
    Requires `ros2 run vision vision_node` in another terminal.
...
# TEMP / TESTING-ONLY: skip-ahead flags (mutually exclusive priority:
# START_AT_CP3 > START_AT_CP2 > full mission). BTN_1 still starts the run.
#
# START_AT_CP2: skip cp1/cp2 scripted drive; reset odom and run the cp2->cp3
#   LAPF weave. Place the robot at the cp2 spot facing +Y first.
# START_AT_CP3: skip cp1/cp2 AND the cp2->cp3 LAPF weave; reset odom and jump
#   straight into the cp3 terminal maneuver (face wall -> approach -> finish
#   straightaway -> cp4 -> cp5). Place the robot at the cp3 approach spot
#   (post-weave, near the outer wall) first.
# >>> Set both False for a full run <<<
START_AT_CP3 = True
START_AT_CP2 = False
...
# ===========================================================================
# CHECKPOINT 5a — STOP-SIGN APPROACH (vision-referenced terminal stop)
# ===========================================================================
# After cp4 the robot creeps forward until the vision node reports a stop sign
# whose pinhole-estimated distance is within STOP_SIGN_STOP_DISTANCE_MM, then
# stops and starts the laser-targeting sequence. Distance comes from the bbox
# width: dist = STOP_SIGN_WIDTH_MM * fx_px / bbox_width_px, with fx scaled from
# the full-resolution camera calibration to the vision frame width.
STOP_SIGN_STOP_DISTANCE_MM = 70.0     # stop when the sign is within this
STOP_SIGN_WIDTH_MM = 60.0             # physical sign width -- MEASURE on the course!
STOP_SIGN_MIN_CONFIDENCE = 0.40       # ignore weaker YOLO detections
STOP_SIGN_CONFIRM_FRAMES = 3          # consecutive in-range FSM ticks before stopping
STOP_SIGN_APPROACH_SPEED_MM_S = 60.0  # slow creep so a 4 Hz vision rate can't overshoot
STOP_SIGN_APPROACH_TIMEOUT_S = 25.0   # give up creeping and proceed to lasing anyway
STOP_SIGN_VISION_STALE_S = 1.5        # treat vision as unavailable beyond this

# Full-resolution camera calibration (4608x2592), scaled to the vision frame.
CAMERA_FX_FULL_PX = 3386.34
CAMERA_FULL_WIDTH_PX = 4608.0

# ===========================================================================
# CHECKPOINT 5b — LASER TARGETING (vision-node-based, closed-loop)
# ===========================================================================
# The vision node (vision/vision_node.py) publishes "aruco marker" detections
# (bbox + marker_id + solvePnP x_m/y_m/z_m attributes) and at most one
# "laser dot" detection per frame into /vision/detections. Everything below
# consumes those via robot.get_detections() -- the camera is never opened here.
#
# Coarse aim: calibration table (Z -> yaw/pitch) + trigonometric offsets, same
# convention as the original lasing prototype (frames flipped 180 deg).
LASER_CALIBRATION_TABLE = [
    (0.75, 94.0, 169.0),
    (1.00, 93.0, 163.5),
    (2.00, 90.5, 157.0),
]
LASER_PITCH_BIAS_DEG = -15.0
LASER_CAMERA_HEIGHT_M = 0.30          # camera lens height from ground
LASER_CALIB_MARKER_HEIGHT_M = 0.20    # calibration marker center height
# In OpenCV camera coords +Y is DOWN, so a marker lower than the camera has a
# positive y_m offset.
LASER_CALIB_Y_OFFSET_M = LASER_CAMERA_HEIGHT_M - LASER_CALIB_MARKER_HEIGHT_M
LASER_BLACKLISTED_IDS = [17]          # false-positive shadows/geometry

# Detection freshness: the vision node processes at ~4 Hz, so after moving a
# servo we must wait for capture + inference + publish before the cached
# detections describe the post-move world.
LASER_VISION_LATENCY_S = 0.6

# Dot validation (poor-man's blink differencing through the vision node): the
# laser is toggled OFF then ON between reads; a "dot" that is still there with
# the laser OFF (within REJECT_PX of the ON position) is a static false
# positive (glint/reflection) and the measurement is rejected.
LASER_STATIC_REJECT_PX = 12.0

# Image-Jacobian visual servo (measured, not assumed): probe each servo at lock
# start to measure how the ERROR responds, then drive corrections with J^-1.
# Handles flip / rotation / axis-swap / sign / scale automatically.
JAC_PROBE_DEG = 4.0                   # servo nudge during calibration
JAC_MIN_DET = 0.30                    # |det(J)| floor (px/deg)^2; below = uncontrollable
JAC_GAIN = 0.55                       # fraction of remaining error corrected per iteration

SERVO_STEP_LIMIT_DEG = 3.0            # max deg change per iteration
SERVO_MAX_DEVIATION_DEG = 25.0        # servos may only roam this far from coarse aim

CENTER_DEADBAND_PX = 4.0              # stop commanding once the dot is this close to center
LOCK_DWELL_FRAMES = 2                 # leaky in-box counter must reach this to fire
SERVO_LOOP_TIMEOUT_S = 45.0           # abort a target (safe) if not locked in time
                                      #   (each validated measurement costs ~1.5 s)
LASER_DWELL_S = 3.0                   # on-target laser dwell ("the shot")
LASER_TARGET_SCAN_TIMEOUT_S = 6.0     # wait this long for markers to appear at cp5
DEBUG_LASER_SERVO = True              # per-iteration console prints
...
def configure_robot(robot: Robot) -> None:
    ...
    # Subscribe to /vision/detections. ALWAYS enabled now (not just for the
    # green-light auto-start): checkpoint 5 needs it for the stop-sign approach
    # and the laser-targeting phase (aruco marker + laser dot detections all
    # come from the vision node). Harmless if the vision node isn't running --
    # detections simply never arrive and cp5 falls back to its timeouts.
    robot.enable_vision()
    print("[sensor] vision enabled (green-light start, cp5 stop sign, laser targeting)")
...
# ===========================================================================
# CHECKPOINT 5 HELPERS — stop-sign approach + vision-node laser targeting
# ===========================================================================
...
def _detection_attr_float(det: dict, name: str) -> float | None:
    """Parse a float attribute published by the vision node, or None."""
    try:
        return float(det["attributes"][name]["value"])
    except (KeyError, TypeError, ValueError):
        return None


def stop_sign_distance_mm(robot: Robot) -> float | None:
    """Pinhole distance estimate to the nearest confident stop sign, or None.

    dist = STOP_SIGN_WIDTH_MM * fx / bbox_width_px, with fx scaled from the
    full-resolution calibration to the actual vision frame width. Returns the
    closest (largest-bbox) candidate; None when vision is stale or no confident
    stop sign is in view.
    """
    if not robot.is_vision_active(timeout_s=STOP_SIGN_VISION_STALE_S):
        return None
    img_w, _img_h = robot.get_detection_image_size()
    if img_w <= 0:
        return None
    fx = CAMERA_FX_FULL_PX * (img_w / CAMERA_FULL_WIDTH_PX)

    best: float | None = None
    for det in robot.get_detections("stop sign"):
        if float(det["confidence"]) < STOP_SIGN_MIN_CONFIDENCE:
            continue
        width_px = int(det["bbox"]["width"])
        if width_px <= 0:
            continue
        dist = STOP_SIGN_WIDTH_MM * fx / float(width_px)
        if best is None or dist < best:
            best = dist
    return best


def laser_get_targets(robot: Robot) -> list[tuple[int, float, float, float]]:
    """All visible ArUco targets as (marker_id, x_m, y_m, z_m), nearest first.

    Reads the vision node's "aruco marker" detections; skips blacklisted IDs
    and markers whose solvePnP pose attributes are missing.
    """
    targets = []
    for det in robot.get_detections("aruco marker"):
        try:
            marker_id = int(det["attributes"]["marker_id"]["value"])
        except (KeyError, TypeError, ValueError):
            continue
        if marker_id in LASER_BLACKLISTED_IDS:
            continue
        x_m = _detection_attr_float(det, "x_m")
        y_m = _detection_attr_float(det, "y_m")
        z_m = _detection_attr_float(det, "z_m")
        if x_m is None or y_m is None or z_m is None:
            continue
        targets.append((marker_id, x_m, y_m, z_m))
    targets.sort(key=lambda t: t[3])
    return targets


def laser_get_marker_px(robot: Robot, target_id: int):
    """(center_xy, bbox) of one marker in vision-frame pixels, or (None, None)."""
    for det in robot.get_detections("aruco marker"):
        try:
            marker_id = int(det["attributes"]["marker_id"]["value"])
        except (KeyError, TypeError, ValueError):
        ...
        return ((x0 + x1) * 0.5, (y0 + y1) * 0.5), (x0, y0, x1, y1)
    return None, None


def laser_get_dot_px(robot: Robot) -> tuple[float, float] | None:
    """Center of the vision node's best "laser dot" detection, or None."""
    best = None
    best_conf = -1.0
    for det in robot.get_detections("laser dot"):
        conf = float(det["confidence"])
        if conf > best_conf:
            b = det["bbox"]
            best = (float(b["x"]) + float(b["width"]) * 0.5,
                    float(b["y"]) + float(b["height"]) * 0.5)
            best_conf = conf
    return best


def laser_measure_dot_validated(shooter, robot: Robot) -> tuple[float, float] | None:
    """Blink-validated laser dot position (vision-frame px), or None.

    The appearance-based dot detector can latch onto glints/reflections, so each
    measurement toggles the laser OFF then ON through the loader and compares:
    a "dot" that persists with the laser OFF (within LASER_STATIC_REJECT_PX of
    the ON position) is static scenery and the measurement is rejected. Leaves
    the laser ON. Each toggle waits LASER_VISION_LATENCY_S for the ~4 Hz vision
    node to publish a post-toggle frame.
    """
    shooter.loader_rest()                 # laser OFF
    time.sleep(LASER_VISION_LATENCY_S)
    dot_off = laser_get_dot_px(robot)

    shooter.loader_push()                 # laser ON (stays on afterwards)
    time.sleep(LASER_VISION_LATENCY_S)
    dot_on = laser_get_dot_px(robot)

    if dot_on is None:
        return None
    if dot_off is not None and math.hypot(
        dot_on[0] - dot_off[0], dot_on[1] - dot_off[1]
    ) <= LASER_STATIC_REJECT_PX:
        if DEBUG_LASER_SERVO:
            print("      [dbg] dot rejected: same spot with laser OFF (static glint)")
        return None
    return dot_on


def laser_measure_error(shooter, robot: Robot, target_id: int, samples: int = 1):
    """Average (marker_center - dot) pixel error, or None if either not seen."""
    exs, eys = [], []
    for _ in range(samples):
        dot = laser_measure_dot_validated(shooter, robot)
        if dot is None:
            continue
        center, _bbox = laser_get_marker_px(robot, target_id)
        if center is None:
            continue
        exs.append(center[0] - dot[0])
        eys.append(center[1] - dot[1])
    if not exs:
        return None
    return (sum(exs) / len(exs), sum(eys) / len(eys))


def laser_interpolate_calibrated_angles(z_target: float) -> tuple[float, float]:
    """Piecewise-linear base yaw/pitch for a given Z-depth (calibration table)."""
    table = sorted(LASER_CALIBRATION_TABLE, key=lambda x: x[0])
    ...
    for i in range(len(table) - 1):
        ...
        if z1 <= z_target <= z2:
            ...
            return y1 + ratio * (y2 - y1), p1 + ratio * (p2 - p1)
    return 93.0, 163.0  # fallback


def laser_calibrate_jacobian(shooter, robot: Robot, target_id: int,
                             yaw0: float, pitch0: float):
    """Measure J = d(error)/d[yaw, pitch] (px/deg) by probing each servo.

    Returns (J_inverse, None) on success or (None, reason) on failure.
    Mount-agnostic: it measures how the ERROR responds, so flips/rotations/
    axis swaps are handled automatically, and a dead feedback loop is caught.
    """
    d = JAC_PROBE_DEG

    shooter.set_yaw(yaw0); shooter.set_pitch(pitch0)
    e0 = laser_measure_error(shooter, robot, target_id)
    if e0 is None:
        return None, "no feedback at nominal aim (marker or dot not detected)"

    shooter.set_yaw(yaw0 + d)
    e_yaw = laser_measure_error(shooter, robot, target_id)
    shooter.set_yaw(yaw0)
    if e_yaw is None:
        return None, "lost feedback during yaw probe"

    shooter.set_pitch(pitch0 + d)
    e_pitch = laser_measure_error(shooter, robot, target_id)
    shooter.set_pitch(pitch0)
    if e_pitch is None:
        return None, "lost feedback during pitch probe"

    J = np.array([
        [(e_yaw[0] - e0[0]) / d, (e_pitch[0] - e0[0]) / d],
        [(e_yaw[1] - e0[1]) / d, (e_pitch[1] - e0[1]) / d],
    ], dtype=np.float64)
    det = J[0, 0] * J[1, 1] - J[0, 1] * J[1, 0]
    print(f"      [JAC] J=[[{J[0,0]:+.1f},{J[0,1]:+.1f}],"
          f"[{J[1,0]:+.1f},{J[1,1]:+.1f}]] det={det:+.2f} px/deg^2")
    if abs(det) < JAC_MIN_DET:
        return None, (f"error barely responds to servo (|det|={abs(det):.2f}); "
                      "feedback not tracking the laser or servo not moving")
    return np.linalg.inv(J), None


def _dot_in_box(dot, bbox) -> bool:
    x0, y0, x1, y1 = bbox
    return x0 <= dot[0] <= x1 and y0 <= dot[1] <= y1


def laser_visual_servo(shooter, robot: Robot, target_id: int,
                       base_yaw: float, base_pitch: float) -> bool:
    """Drive the laser dot onto the marker center with J^-1 corrections.

    Lock = blink-validated dot inside the marker bbox for LOCK_DWELL_FRAMES
    (leaky counter); then the laser dwells LASER_DWELL_S (the shot).
    Returns True if it fired, False if it aborted (safe: laser off).
    """
    # Track what the hardware ACTUALLY accepted (Shooter clamps internally), so
    # the loop never thinks it moved while pinned against a servo limit.
    yaw = shooter.set_yaw(base_yaw)
    pitch = shooter.set_pitch(base_pitch)

    j_inv, reason = laser_calibrate_jacobian(shooter, robot, target_id, yaw, pitch)
    if j_inv is None:
        print(f"      [SERVO] Calibration failed: {reason}. Aborting (safe).")
        shooter.loader_rest()
        return False
    ...
    while True:
        if time.time() - t_start > SERVO_LOOP_TIMEOUT_S:
            print("      [SERVO] Timeout — no lock. Aborting target (safe).")
            shooter.loader_rest()
            return False
        ...
        if dot is None:
            if DEBUG_LASER_SERVO:
                print("      [dbg] laser dot NOT detected/validated this cycle")
            continue
        center, marker_bbox = laser_get_marker_px(robot, target_id)
        if center is None:
            if DEBUG_LASER_SERVO:
                print(f"      [dbg] marker {target_id} NOT seen this cycle")
            continue

        ex = center[0] - dot[0]
        ey = center[1] - dot[1]
        loss = math.hypot(ex, ey)             # control objective: distance to center
        in_box = _dot_in_box(dot, marker_bbox)  # lock objective: inside the marker

        # Leaky lock counter: tolerate an occasional stray out-frame.
        lock_score = min(lock_score + 1, LOCK_DWELL_FRAMES) if in_box \
            else max(lock_score - 1, 0)

        if DEBUG_LASER_SERVO:
            print(f"      [dbg] dot=({int(dot[0])},{int(dot[1])}) "
                  f"{'IN-BOX' if in_box else 'OUT'} loss={loss:.0f} "
                  f"yaw={yaw:.1f} pitch={pitch:.1f} lock={lock_score}")

        if lock_score >= LOCK_DWELL_FRAMES:
            print(f"      [SERVO] Locked inside marker box (loss={loss:.0f}px). "
                  f"Holding shot for {LASER_DWELL_S:.0f}s.")
            time.sleep(LASER_DWELL_S)  # laser is already ON == the shot
            shooter.loader_rest()
            return True

        # Drive toward CENTER; deadband suppresses micro-jitter near center.
        if loss > CENTER_DEADBAND_PX:
            du = -JAC_GAIN * (j_inv @ np.array([ex, ey], dtype=np.float64))
            d_yaw = _clamp(float(du[0]), -SERVO_STEP_LIMIT_DEG, SERVO_STEP_LIMIT_DEG)
            d_pitch = _clamp(float(du[1]), -SERVO_STEP_LIMIT_DEG, SERVO_STEP_LIMIT_DEG)
            yaw = shooter.set_yaw(_clamp(
                yaw + d_yaw,
                base_yaw - SERVO_MAX_DEVIATION_DEG, base_yaw + SERVO_MAX_DEVIATION_DEG))
            pitch = shooter.set_pitch(_clamp(
                pitch + d_pitch,
                base_pitch - SERVO_MAX_DEVIATION_DEG, base_pitch + SERVO_MAX_DEVIATION_DEG))


def run_laser_targeting(robot: Robot) -> None:
    """Checkpoint 5: sequential closed-loop lasing of every visible ArUco marker.

    Vision-node-driven end to end (no direct camera access). Blocks until done;
    acceptable because the robot is already stopped at the stop sign. Imported
    lazily so a missing/incomplete manipulator never crashes the mission.
    """
    print("[FSM] CHECKPOINT 5 - autonomous laser targeting (vision node)")
    try:
        from robot.shooter import Shooter

        shooter = Shooter(robot)
        shooter.enable()
        shooter.loader_rest()  # laser starts OFF
        try:
            # Wait briefly for the vision node to report targets.
            t0 = time.time()
            targets: list[tuple[int, float, float, float]] = []
            while time.time() - t0 < LASER_TARGET_SCAN_TIMEOUT_S:
                targets = laser_get_targets(robot)
                if targets:
                    break
                time.sleep(0.25)

            if not targets:
                print("[FSM] cp5: no ArUco targets visible - skipping lasing "
                      "(is the vision node running with laser_targets_enabled?)")
                return

            order = [t[0] for t in targets]
            print(f">>>>>>>> LASING {len(targets)} TARGET(S), near->far: {order} <<<<<<<<")

            for seq, (t_id, x_m, y_m, z_m) in enumerate(targets):
                print(f"\n[TARGET #{seq + 1}] ID {t_id} | Z {z_m:.2f} m")

                # --- 1. coarse open-loop aim (gets the dot into capture range)
                base_yaw, base_pitch = laser_interpolate_calibrated_angles(z_m)
                y_dev = y_m - LASER_CALIB_Y_OFFSET_M
                yaw_off = math.degrees(math.atan2(x_m, z_m))
                pitch_off = math.degrees(math.atan2(y_dev, z_m))
                coarse_yaw = shooter.set_yaw(base_yaw - yaw_off)
                coarse_pitch = shooter.set_pitch(
                    base_pitch + pitch_off + LASER_PITCH_BIAS_DEG)
                time.sleep(0.5)

                # --- 2. closed-loop refine + fire
                print("      ---> Coarse aim set. Engaging visual servo...")
                fired = laser_visual_servo(shooter, robot, t_id, coarse_yaw, coarse_pitch)
                print(f"      ---> {'Shot complete' if fired else 'Skipped (no lock)'} "
                      f"on ID {t_id}.")
                time.sleep(0.5)

            print("\n[FSM] cp5 lasing sequence finished.")
        finally:
            shooter.loader_rest()
            shooter.disable()
    except Exception as exc:  # noqa: BLE001 - cp5 must never crash the mission
        print(f"[warn] cp5 laser targeting aborted: {exc}")
...
def run(robot: Robot) -> None:
    ...
    while True:
        ...
        if state == "INIT":
            ...
            if START_AT_CP3:
                print("[CFG] TEMP START_AT_CP3 - will skip to cp3 terminal on BTN_1")
            elif START_AT_CP2:
                print("[CFG] TEMP START_AT_CP2 - will skip to cp2->cp3 LAPF on BTN_1")
            state = "IDLE"
            ...
            if button_start or green_start:
                ...
                if START_AT_CP3:
                    # TEMP/testing path: skip cp1/cp2 and the cp2->cp3 LAPF weave;
                    # jump straight into the cp3 terminal maneuver. Place the
                    # robot at the cp3 approach spot (post-weave, near the outer
                    # wall) before pressing BTN_1.
                    reset_mission_pose(robot)
                    show_running_leds(robot)
                    step_index = len(MISSION_STEPS)
                    av = {}
                    print("[FSM] TEMP START_AT_CP3 - skipping to cp3 terminal maneuver")
                ...
                if motion_handle is not None and motion_handle.is_finished():
                    ...
                    if remaining <= OBSTACLE_AVOIDANCE_TOLERANCE_MM + SUCCESS_MARGIN_MM:
                        ...
                        if next_state == "CP3_FACE_WALL":
                            ...
                            last_status_print_at = now
                            state = "CP3_FACE_WALL"
                        else:  # CP5_APPROACH_STOP_SIGN
                            print("[FSM] CHECKPOINT 4 reached - approaching the cp5 stop sign "
                                  f"(stop within {STOP_SIGN_STOP_DISTANCE_MM:.0f} mm)")
                            robot.stop()
                            motion_handle = None
                            av["stop_sign_started_at"] = now
                            av["stop_sign_inrange_count"] = 0
                            last_status_print_at = now
                            state = "CP5_APPROACH_STOP_SIGN"
                    else:
            ...
            if robot.was_button_pressed(Button.BTN_2):
                ...
                if remaining <= 0.0:
                    ...
                    motion_handle = begin_avoidance_segment(
                        ...
                        next_state="CP5_APPROACH_STOP_SIGN",
                        now=now,
                    )
                    last_status_print_at = now
                    state = "OBSTACLE_AVOIDANCE"
        ...
        elif state == "CP5_APPROACH_STOP_SIGN":
            # Creep forward until the vision node reports a stop sign within
            # STOP_SIGN_STOP_DISTANCE_MM (pinhole estimate from the bbox width),
            # confirmed over STOP_SIGN_CONFIRM_FRAMES consecutive ticks so one
            # noisy bbox can't stop early. Timeout = stop where we are and run
            # the lasing anyway (the shot matters more than the last few mm).
            if robot.was_button_pressed(Button.BTN_2):
                robot.stop()
                motion_handle = None
                show_idle_leds(robot)
                print("[FSM] IDLE - cp5 stop-sign approach cancelled")
                state = "IDLE"
            else:
                dist = stop_sign_distance_mm(robot)

                if now - last_status_print_at >= STATUS_PRINT_INTERVAL_S:
                    dist_str = "none" if dist is None else f"{dist:.0f} mm"
                    print(
                        f"  cp5 stop-sign approach: sign={dist_str} "
                        f"(stop at {STOP_SIGN_STOP_DISTANCE_MM:.0f} mm, "
                        f"in-range {av.get('stop_sign_inrange_count', 0)}/"
                        f"{STOP_SIGN_CONFIRM_FRAMES})"
                    )
                    last_status_print_at = now

                if dist is not None and dist <= STOP_SIGN_STOP_DISTANCE_MM:
                    robot.stop()
                    av["stop_sign_inrange_count"] = av.get("stop_sign_inrange_count", 0) + 1
                    if av["stop_sign_inrange_count"] >= STOP_SIGN_CONFIRM_FRAMES:
                        print(
                            f"[FSM] cp5 stop sign confirmed at {dist:.0f} mm "
                            f"(<= {STOP_SIGN_STOP_DISTANCE_MM:.0f} mm) - stopping for lasing"
                        )
                        state = "CP5_LASER_TARGETING"
                elif (now - av.get("stop_sign_started_at", now)) >= STOP_SIGN_APPROACH_TIMEOUT_S:
                    robot.stop()
                    print(
                        "[FSM] WARN cp5 stop-sign approach timed out "
                        f"({STOP_SIGN_APPROACH_TIMEOUT_S:.0f} s) - no sign within "
                        f"{STOP_SIGN_STOP_DISTANCE_MM:.0f} mm; stopping here and lasing anyway"
                    )
                    state = "CP5_LASER_TARGETING"
                else:
                    av["stop_sign_inrange_count"] = 0
                    robot.set_velocity(STOP_SIGN_APPROACH_SPEED_MM_S, 0.0)

        elif state == "CP5_LASER_TARGETING":
            # Robot is stopped at the stop sign. Run the full vision-node-driven
            # lasing sequence (BLOCKING -- BTN_2 is not polled during the shots,
            # acceptable because the base is stationary), then go idle.
            robot.stop()
            motion_handle = None
            run_laser_targeting(robot)
            show_idle_leds(robot)
            print("[FSM] IDLE - mission complete, press BTN_1 to run again")
            state = "IDLE"
